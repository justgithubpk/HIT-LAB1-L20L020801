...
public class TurtleSoup {
	...
	public static Set<Point> convexHull(Set<Point> points) {
    	...
    	point.addAll(points);  //set a new set point to prevent from changing original point
    	int length;
    	length = point.size();
    	if(length <= 3) {
    		return points;
    		...
    		Point start = point.get(0);  //1st make the bottom-left point as starting point
    		for(Point t:points) {
    			if(t.x() < start.x()) {
    				start = t;
    			}
    		...
    		tubao.add(start); //add the bottom-left point to convex point set
    		int i=0;
    		point.remove(start);  //remove starting point from original set
    		Point forpoint = start;
    		do {
    			if(i==1) {
    				point.add(start);  //re-add starting point to check ending
    			}
    			double toangle = 360, todif = 0;
    			Point topoint = null;  //points await adding
    			for(Point temp:point) {
    				double tempdushu = calculateBearingToPoint(0,(int)forpoint.x(),(int)forpoint.y(),(int)temp.x(),(int)temp.y());
    				double tempdistance = Math.pow(forpoint.x() - temp.x(), 2) + Math.pow(forpoint.y() - temp.y(), 2); //calculate distance
    				if(tempdushu < toangle) {  //find smallest deflection angle
    					toangle = tempdushu;
    					topoint = temp;
    					todif = tempdistance;
    				}
    				else if(tempdushu==toangle && tempdistance>todif) { //if same height, find the furthest point
    					topoint = temp;
    					todif = tempdistance;
    				}	
    			}
    ...
    public static void drawPersonalArt(Turtle turtle) {
        //throw new RuntimeException("implement me!");
    	PenColor[] color = new PenColor[6];
    	color[0] = PenColor.BLACK;
    	color[1] = PenColor.BLUE;
    	color[2] = PenColor.MAGENTA;
    	color[3] = PenColor.GRAY;
    	color[4] = PenColor.ORANGE;
		color[5] = PenColor.RED;
    ...
    public static void main(String args[]) {
		...
		drawPersonalArt(turtle); //draw a flower
        turtle.draw();
    }
